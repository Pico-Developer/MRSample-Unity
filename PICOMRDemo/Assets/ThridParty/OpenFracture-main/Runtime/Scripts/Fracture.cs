...
public class Fracture : MonoBehaviour
{
    ...
    // 添加该值是为了同一帧多次碰撞造成不断的破碎，从而导致破碎的mesh会有问题
    ...
    public void TryFractureByCollision(Collision collision)
    {
    ...
    {
        if (triggerOptions.triggerType == TriggerType.Collision && !_isFractured)
        {
        ...
        if (mesh != null)
        {
            ...
            if (fractureOptions.asynchronous)
            {
                StartCoroutine(Fragmenter.FractureAsync(
                    ...
                    () =>
                    {
                        OnFractureCompleted(fragmentTemplate);
                    }
                ));
            }
            else
    ...
    private void OnFractureCompleted(GameObject fragmentTemplate)
    {
