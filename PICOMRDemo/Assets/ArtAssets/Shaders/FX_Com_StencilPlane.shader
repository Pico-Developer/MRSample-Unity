	...
	SubShader
	{
		...
		Pass
		{
						 ...
						 ) : SV_Target
			{
				...
				float3 unpack25 = UnpackNormalScale( tex2D( _Normal_Tex, uv_Normal_Tex ), _Normal_Strength );
				unpack25.z = lerp( 1, unpack25.z, saturate(_Normal_Strength) );
				
				float2 uv_Mask = IN.ase_texcoord8.xyz.xy * _Mask_ST.xy + _Mask_ST.zw;
				

				float3 BaseColor = ( ( _Color_Cube * texCUBE( _CubeMap, ase_tanViewDir ) ) + ( ( _Color_Noise * tex2D( _Noise, uv_Noise ) ) * _Noise_Strength ) ).rgb;
				float3 Normal = ( ase_grabScreenPosNorm * float4( unpack25 , 0.0 ) ).xyz;
				float3 Emission = 0;
				float3 Specular = 0.5;
				float Metallic = 0;
...
ASEEND*/
//CHKSM=78C458EEC52D70AD47498C3E4C5BEFEEA2105D76